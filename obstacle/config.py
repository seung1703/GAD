...
DEFAULTS = {
    ...
    # 감지 즉시 최대조향+PWM으로 꺾기 시작 (정지 대기 단계 없음).
    #   꺾기(OUT)   종료 = change_t_out 고정시간 오픈루프 (대각선 이동 중이라
    #                     차선 판정이 흔들리므로 비전 미사용, 시간으로만 진행)
    #   정렬(ALIGN) 종료 = |헤딩| < align_heading_thresh, 초과 시 change_t_max 폴백
    "change_t_out": 1.0,      # 꺾기 단계 고정 지속시간(초) — 위 기하 계산 참고
    "change_t_max": 2.5,      # 정렬(ALIGN) 타임아웃 폴백(초) — 헤딩 수렴 미달성 시 강제 진행
    ...
    "bev_lane_width": 328,   # 단일 차선 시 offset에 쓸 고정 차로폭(BEV px). 라이브
                             # 측정은 커브서 팽창/가장자리서 축소돼 불안정 → 고정값.
    "lookahead_exit_scale": 0.4,  # 코너 탈출(중앙 복귀) 국면에 룩어헤드 축소 배율.
                             # 낮을수록 탈출 오버슈트↓, 1.0=비대칭 끔
