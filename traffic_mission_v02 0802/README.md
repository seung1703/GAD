...
1. `model/best_car.pt`가 신호등 카메라의 `car_roi_*` 영역에서 `obstacle-car`를 검출하고, 같은 시점의 새 전방 초음파 측정값이 `obs_trigger_cm`보다 가까우면 첫 융합 프레임부터 속도를 `change_pwm` 이하로 제한합니다. 이 상태를 `obs_trigger_hits`회 확인하면 회피에 진입합니다.
...
9. 회복 가속 중에도 즉시 장애물 감지를 다시 시작합니다. 새 자동차+근거리 초음파 융합 측정 2회와 dash 방향이 확인되면 가속 램프보다 새 회피를 우선합니다.
...
- 새 측정 확인 횟수: `obs_trigger_hits = 2`
- 근접 감속 거리: `obstacle_slow_cm = 80`
- 근접 감속 속도: `obstacle_slow_pwm = 100`
...
모델 `red` 클래스 재출발이 한 번 완료되면 신호 미션 상태는 `COMPLETE`가 되고 신호등 YOLO 추론을 더 이상 호출하지 않습니다. `Traffic ROI` 창에는 `TRAFFIC MISSION COMPLETE - DETECTOR OFF`가 표시되며 이후 신호등에는 반응하지 않습니다. 자동차 후면 모델은 같은 카메라의 `car_roi_*` 영역에서 별도 스레드로 계속 동작하므로 장애물 회피 기능은 유지됩니다. 신호 미션을 다시 수행하려면 `RESET` 버튼 또는 `r` 키로 전체 런타임 상태를 초기화한 뒤 `s`를 누릅니다.

## ROI UI

`Integrated Drive` 창의 트랙바에서 BEV 사다리꼴과 중앙값을 실시간 조절할 수 있습니다. 별도의 `Traffic ROI` 창에서는 신호등 ROI와 자동차 ROI를 함께 실시간 조절합니다.
...
| 자동차 모델 | `car_detected`, `car_confidence`, `car_bbox`, `car_area_ratio`, `car_result_age_s`, `car_roi_normalized`, `car_roi_pixels`, `car_model_fault` |
...
| 차선·신호 | `lane_found`, `stop_line_detected`, `traffic_signal`, `traffic_result_age_s`, `traffic_model_fault`, `traffic_mission_enabled`, `signal_stop_latched`, `avoidance_blocked_by_signal`, `lost_frames`, `vehicle_state` |
...
│  ├─ car_detector.py              비동기 best_car.pt 자동차 ROI 추론
...
`change_steer = 1.0`, `change_duration_s = 2.2`, `counter_steer_duration_s = 1.0`, `change_pwm = 70`은 강한 개방루프 조향입니다. 자동차+초음파가 처음 융합되는 순간부터 속도를 `70` 이하로 제한하고, 회피 중에는 추가 거리 변화로 속도를 바꾸지 않습니다. 완료 후에는 `70 → 255`로 3초 동안 올리지만 새 장애물이 검출되면 다시 `70` 제한과 회피가 우선합니다. 반드시 바퀴를 띄운 dry-run 확인과 저속 공간 시험 후 실제 코스에서 사용해야 합니다. solid가 약하면 우선 원본 화면의 `raw S`와 `roi S`를 비교한 뒤 `model_conf`, `model_mask_threshold`, `mask_roi_margin_x` 순서로 조절합니다.
