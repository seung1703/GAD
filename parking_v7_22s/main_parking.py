...
def run_mission(reader, car, viz, sim=False, back=False):
    ...
    fsm.back_mode = back
    if back:
        print(f"[mission] --back : 우회전 뒤 {C.BACK_FWD_S:.1f}s 직진하고 "
              f"→ 후진으로 바뀝니다 (후방 비상정지는 후진 구간부터 동작)")
    ...
    # ★SEARCH 시계를 여기서 다시 잡는다★
    # t_state 는 ParkingFSM 을 만들 때 잡히는데, 그건 's' 대기와 호밍보다
    # **앞**이다. 초기 상태가 이미 SEARCH 라 _go() 로 갱신될 기회도 없다.
    # 그래서 차 자리 잡느라 SEARCH_TIMEOUT_S(50초)를 넘겨 's' 를 누르면,
    # 워밍업이 끝나는 순간 곧바로 "SEARCH 타임아웃" 이 났다. (실측 재현됨)
    # 이제 SEARCH 타임아웃은 **'s' 를 누른 시점부터** 센다.
    fsm.t_state = time.time()
    ...
    try:
        while not _CTRL["stop"]:
            ...
            fsm.update(per)
            # 카메라 디버그 창 — ★메인 스레드에서만★ (macOS 제약).
            # 가이드 스레드는 그리기만 하고 여기서 띄운다.
            if C.GUIDE_SHOW and guide.view is not None:
                cv2.imshow("parking cam", guide.view)
                cv2.waitKey(1)
            if viz:
                viz.update(per, fsm.state, fsm.status,
                           sectors=fsm.sector_names())
