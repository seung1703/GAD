...
_ROT_ALIVE_DEG = 0.5         # 회전량이 이만큼이라도 늘면 "측정이 살아 있다".
                             #   정체 폴백(SWING_ROT_STALL_S)의 판단 기준.
                             #   풀락 스윙의 실제 회전은 10~25도/초라, 0.5도는
                             #   아주 느슨한 생존 신호다
...
class _RotChannel:
    """물체 하나의 **면 방향**으로 회전량을 재는 채널.

    회전량 = (지금까지 누적) + |현재 면각 − 이 구간의 기준 면각|

    ★핵심: 불연속에서 멈추지 않는다★
      추적기가 다른 물체로 갈아타거나(재획득) 보이는 면이 앞면↔옆면으로
      바뀌면, 기준 면각은 옛 것이라 더는 못 쓴다. 예전 코드는 그런 프레임을
      **버리기만** 해서, 새 물체의 면각이 계속 기준에서 멀면 영영 회복하지
      못하고 회전량이 얼어붙었다(실차에서 23도에 고정).

      여기서는 불연속이 보이면 그 자리를 **새 기준으로 다시 잡고**(anchor),
      그때까지의 누적 회전량을 offset 으로 넘긴다. 잃는 것은 갈아타는
      그 한 프레임(≈50ms) 분의 회전뿐이다.
    """
    ...
    def reset(self):
        self.a0 = None        # 이 구간의 기준 면각
        self.aprev = None     # 직전 프레임 면각 (급변 감지용)
        self.off = 0.0        # 이전 구간들에서 넘어온 누적 회전량
        self.rebases = 0      # 기준선을 다시 잡은 횟수 (진단용)

    def _anchor(self, a, acc):
        self.a0 = a
        self.aprev = a
        self.off = acc

    def update(self, a, acc, switched):
        """a=이번 프레임 면각(None이면 못 봄), acc=현재까지 확정 회전량,
        switched=추적기가 다른 물체를 물었나. → 이 채널이 보는 회전량 또는 None."""
        if a is None:
            return None
        if self.a0 is None:                       # 최초 앵커
            self._anchor(a, acc)
            return self.off
        # 불연속 — 재획득이거나, 프레임간 면각이 튀었거나(앞면↔옆면은 ~90도)
        if switched or P.angle_between(a, self.aprev) > _SWING_JUMP_MAX:
            self._anchor(a, acc)                  # ★버리지 않고 다시 센다★
            self.rebases += 1
            return self.off
        self.aprev = a
        return self.off + P.angle_between(a, self.a0)
...
class ParkingFSM:
    ...
    def __init__(self, car, guide=None):
        ...
        # 회전량 측정 채널 — 1번차/2번차 각각. 어느 쪽이든 가만히 있는 물체라
        # 내가 돌아간 각도는 같게 나온다. 1번차가 멀어져 안 보이면 2번차가 잇는다.
        # (실측: 1번차가 2600~3000mm 경계에서 들락날락하며 회전량이 32에서
        #  멈춰 목표 35를 영영 못 넘고 타임아웃까지 간 적이 있다)
        self._rot1 = _RotChannel()
        self._rot2 = _RotChannel()
        self._rot_reacq1 = 0         # 재획득 카운터의 직전값 — 바뀌면 "다른 물체"
        self._rot_reacq2 = 0
        self._rot_grow_t = None      # 회전량이 마지막으로 **늘어난** 시각.
                                     #   이게 오래 안 바뀌면 측정이 죽은 것이다
        self.rot_src = "-"           # 지금 어느 차로 재고 있나 (표시용)
        self._wheels_centered = False
        self.status = ""
        self.emergency = False
        self.back_mode = False       # --back: 마지막 CRUISE 를 후진으로
        self.back_rev = False        # 그 --back 크루즈가 **후진 구간에 들어갔나**.
                                     #   앞의 BACK_FWD_S 초는 전진이라, 그동안
                                     #   후방 감시를 켜면 뒤에 뭐가 있을 때
                                     #   전진 중에 멈춰 선다
    ...
    def shift_clock(self, dt):
        ...
        if self._align_t0 is not None:
            self._align_t0 += dt
        # 이것도 안 밀면, 스윙 중 일시정지했다 재개하는 순간 "회전 정체"로
        # 오판해서 스윙이 바로 끝나버린다
        if self._rot_grow_t is not None:
            self._rot_grow_t += dt
    ...
    def _swing_rotation(self):
        ...
        ★어떻게 재나★
          추적 중인 차의 **면 방향**을 기준 시점과 비교한다. 그 차는 가만히
          있으므로, 면 방향이 바뀐 만큼이 곧 내 차가 돌아간 각도다. 절대 기준이
          필요 없어서 어느 면을 보고 있든(앞면/옆면) 상관없다.

        ★불연속은 '거부'가 아니라 '재앵커'로 다룬다★ (v7 수정)
          예전에는 추적기가 다른 물체로 갈아타거나 면이 바뀌면 그 프레임을
          **버리기만** 했다. 그런데 기준선(a0/aprev)은 옛 물체 것 그대로라,
          새 물체의 면각이 계속 기준에서 멀면 매 프레임 버려져서
          **회전량이 영구히 얼어붙었다.**
          실측 로그: 회전이 23도에서 멈춘 채 3.5초를 더 전진해 2번차를 지나침.
          (`재획득 0→1→2`, `dist 1761→2806` 이 그 증거였다)
          이제는 불연속이 보이면 **그 자리에서 기준선을 새로 잡고 계속 센다.**
          지금까지 쌓은 회전량은 offset 으로 넘기므로 잃지 않는다.
        """
        def face(tr):
            c = tr.cluster
        ...
        acc = self.swing_rot or 0.0

        # 추적기가 이번 프레임에 **다른 물체를 물었나**. TrackedCar 는 게이트
        # (_TRACK_GATE_MM) 밖에서 새로 고를 때만 reacquired 를 올린다 =
        # "같은 물체라고 보장할 수 없다" 는 신호다.
        sw1 = self.car_track.reacquired != self._rot_reacq1
        sw2 = self.car2_track.reacquired != self._rot_reacq2
        self._rot_reacq1 = self.car_track.reacquired
        self._rot_reacq2 = self.car2_track.reacquired

        a1, a2 = face(self.car_track), face(self.car2_track)

        # 2번차 채널도 볼 수 있을 때 미리 앵커해 둔다. 1번차가 아직 멀쩡할 때
        # 잡아야 offset(그 시점의 회전량)이 정확하다.
        r1 = self._rot1.update(a1, acc, sw1)
        r2 = self._rot2.update(a2, acc, sw2)

        # 1번차를 우선 쓰고, 못 재면 2번차로 이어 잰다. 둘 다 가만히 있는
        # 물체라 내가 돌아간 각도는 어느 쪽으로 재든 같다.
        if r1 is not None:
            rot, self.rot_src = r1, "1"
        elif r2 is not None:
            rot, self.rot_src = r2, "2"
        else:
            return self.swing_rot               # 이번 프레임엔 못 잼

        # 회전은 되돌아가지 않는다. 줄어드는 건 잡음이므로 최댓값을 유지
        prev = self.swing_rot
        self.swing_rot = max(acc, float(rot))
        # 측정이 살아 있는지(=늘고 있는지) 감시한다. 아래 정체 폴백이 쓴다.
        if prev is None or self.swing_rot > prev + _ROT_ALIVE_DEG:
            self._rot_grow_t = time.time()
    ...
    def rear_guard(self, scan):
        ...
        # ★단, 실제로 후진 중일 때만★ --back 은 앞의 BACK_FWD_S 초 동안
        # 전진하는데, 그 구간까지 후방을 보면 뒤에 벽이 있을 때 전진을 못 한다.
    ...
    def update(self, percep):
        ...
        if self.state == self.SEARCH:
            ...
            # 탐색은 빠르게. 여기는 첫 차를 찾으러 가는 구간이라 정밀도가
            # 필요 없다. 측정·정밀기동 구간(APPROACH/SWING_OUT)은 FWD_SPEED.
            self.car.motor_forward(C.SEARCH_SPEED)
            ...
            if self._elapsed() >= C.APPROACH_S:
                ...
                self.car.motor_stop()
                self.car2_track.reset()    # ★먼저★ reset 이 reacquired 를 0 으로 돌린다
                # 회전량 기준을 여기서 새로 잡는다. 재획득 카운터의 기준값도
                # 지금 값으로 맞춰 둬야 첫 프레임이 "갈아탐"으로 오인되지 않는다.
                self._rot1.reset()
                self._rot2.reset()
                self._rot_reacq1 = self.car_track.reacquired
                self._rot_reacq2 = self.car2_track.reacquired
            ...
            # ★회전 정체 폴백 — 마지막 안전망★
            # 전진은 충분한데(advanced) 회전량이 SWING_ROT_STALL_S 동안 조금도
            # 안 늘면, 그건 차가 안 도는 게 아니라 **측정이 죽은 것**이다.
            # 풀락 스윙의 실제 회전은 10~25도/초라 1초 넘게 0도일 수가 없다.
            # 그대로 두면 8초 타임아웃까지 계속 전진하며 2번차를 지나친다.
            # (실측: 회전 23도에 고정된 채 3.5초를 더 갔다)
            stalled = False
            if (not done and advanced and C.SWING_ROT_STALL_S > 0
                    and self._rot_grow_t is not None
                    and time.time() - self._rot_grow_t > C.SWING_ROT_STALL_S):
                stalled = True
            ...
            if turned and not advanced:
                ...
                stall_s = (0.0 if self._rot_grow_t is None
                           else time.time() - self._rot_grow_t)
                wait += f"  [회전 대기 {stall_s:.1f}/{C.SWING_ROT_STALL_S:.1f}s]"
            self.status = (f"swing_out  회전 {rot_txt}/{C.SWING_STOP_DEG:.0f}도"
                           ...
                           f"/{self.car2_track.reacquired}"
                           f"  재앵커 {self._rot1.rebases}/{self._rot2.rebases}{wait}")
            if done or stalled:
                if stalled:
                    print(f"[fsm] ☆회전 정체☆ 회전이 {C.SWING_ROT_STALL_S:.1f}s 동안 "
                          f"{rot_txt} 에서 안 늘었습니다 (목표 "
                          f"{C.SWING_STOP_DEG:.0f}). 전진 조건은 이미 충족 —\n"
                          f"       측정 실패로 보고 여기서 스윙을 끝냅니다 "
                          f"(재획득 {self.car_track.reacquired}"
                          f"/{self.car2_track.reacquired}, 재앵커 "
                          f"{self._rot1.rebases}/{self._rot2.rebases})")
            ...
            self.car.steer_center()
            self.car.motor_forward(C.EXIT_SPEED)
            ...
            # 우 풀락으로 돈다. 조향은 카메라가 아니라 여기서 정한다 —
            # 카메라는 "다 돌았나" 판정에만 쓴다.
            self.car.steer_full_right()
            self.car.motor_forward(C.EXIT_SPEED)

            ang = None if self.guide is None else self.guide.angle
            ref = None if self.guide is None else self.guide.straight_angle
            done = False
            # ★먼저 살짝 돈다★ 그 전에는 비전을 아예 안 본다.
            blind = self._elapsed() < C.EXIT_TURN_MIN_S
            if blind:
                self._hold_t0 = None      # 홀드가 미리 쌓이지 않게
                self.status = (f"exit_turn  [선회 중 {self._elapsed():.1f}/"
                               f"{C.EXIT_TURN_MIN_S:.1f}s — 비전 대기]")
            elif C.EXIT_TURN_MODE == "vision" and ref is not None:
                # ★SEARCH 때 보던 각도로 돌아오면 회전 완료★
                # 후진 진입에서 좌로 90도 돌았으니 탈출에서 우로 90도 돌면
                # 원래 진행방향이다 = 그 선이 같은 각도로 다시 보인다.
                near = ang is not None and abs(ang - ref) <= C.EXIT_TURN_TOL
                done = self._hold(near, C.EXIT_TURN_HOLD_S)
                self.status = (f"exit_turn  각 "
                               f"{'--' if ang is None else f'{ang:+.0f}'}"
                               f"→{ref:+.0f}±{C.EXIT_TURN_TOL:.0f}도"
                               f"  {self._elapsed():.1f}/{C.EXIT_TURN_S:.1f}s")
                if done:
                    print(f"[fsm] 탈출 정렬(각 {ang:+.0f}도, 기준 {ref:+.0f}) "
                          "→ 선 따라 주행")
            elif not blind:
                self.status = f"exit_turn {self._elapsed():.1f}/{C.EXIT_TURN_S:.1f}s"

            # 시간은 항상 안전 폴백으로 남는다 (비전이 안 걸려도 끝나야 한다)
            if done or self._elapsed() >= C.EXIT_TURN_S:
                if not done and C.EXIT_TURN_MODE == "vision":
                    print(f"[fsm] 탈출 회전 시간 초과 — 각 "
                          f"{'--' if ang is None else f'{ang:+.0f}'}"
                          f" (기준 {'--' if ref is None else f'{ref:+.0f}'}), 그대로 진행")
                self._go(self.CRUISE)

        elif self.state == self.CRUISE:
            # --back 이면 **잠깐 더 나갔다가 후진**한다. 디버깅용 — 한 판 끝나고
            # 차를 출발 지점 쪽으로 되돌려 다음 시도를 바로 할 수 있다.
            #
            # ★왜 곧바로 후진하지 않나★
            #   우회전이 막 끝난 지점은 아직 슬롯 입구 바로 앞이다. 거기서
            #   그대로 뒤로 가면 방금 나온 자리로 되돌아간다. BACK_FWD_S 만큼
            #   더 빠져나온 뒤에 후진해야 출발선 쪽으로 간다.
            if self.back_mode:
                if self._elapsed() < C.BACK_FWD_S:
                    # 직진 구간 — 평소 크루즈와 똑같이 카메라로 선을 따라간다.
                    # (선을 못 보면 _drive_straight() 가 중립 직진으로 돈다)
                    self.back_rev = False
                    cam = self._drive_straight()
                    self.car.motor_forward(C.EXIT_SPEED)
                    self.status = (f"cruise → 직진 {self._elapsed():.1f}/"
                                   f"{C.BACK_FWD_S:.1f}s (--back){cam}")
                else:
                    # 후진 구간 — 바퀴를 중립으로 놓고 뒤로. 카메라는 안 쓴다
                    # (뒤를 보는 카메라가 아니다). 여기서부터 후방 감시가 켜진다.
                    if not self.back_rev:
                        print(f"[fsm] --back : 직진 {C.BACK_FWD_S:.1f}s 완료 "
                              f"→ 후진 시작 (후방 감시 ON, ESC 로 정지)")
                    self.back_rev = True
                    self.car.steer_center()
                    self.car.motor_reverse(C.REV_SPEED)
                    self.status = "cruise ← 후진 (--back, ESC 로 정지)"
            else:
                # 비전 모드면 그 선을 따라 나간다 (SEARCH 때와 같은 제어).
                # 선을 못 보면 _drive_straight() 가 알아서 중립 직진으로 돈다.
                cam = self._drive_straight()
                self.car.motor_forward(C.EXIT_SPEED)
                self.status = f"cruise — 직진 (ESC 로 정지){cam}"
