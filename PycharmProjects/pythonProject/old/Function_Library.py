    ...
    def initial_setting(self, cam0port=0, cam1port=1, capnum=1):
        ...
        if capnum == 1:
            ...
            if channel0.isOpened():
                channel0.set(cv2.CAP_PROP_BUFFERSIZE, 1)
                channel0.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)
                channel0.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)
                channel0.set(cv2.CAP_PROP_FPS, 30)
                channel0.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*'MJPG'))
            ...
            if channel1.isOpened():
                channel1.set(cv2.CAP_PROP_BUFFERSIZE, 1)
                channel1.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)
                channel1.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)
                channel1.set(cv2.CAP_PROP_FPS, 30)
                channel1.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*'MJPG'))
